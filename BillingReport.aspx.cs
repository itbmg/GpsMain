...
public partial class BillingReport : System.Web.UI.Page
{
    ...
    protected void btn_generate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            lbl_nofifier.Text = "";
            Panel3.Visible = true;
            lbl_routename.Text = ddl_routes.SelectedItem.Text;
            if (startdate.Text != "" && enddate.Text != "")
            {
                ...
                if (ddl_routes.SelectedIndex == 0)
                {
                    if (ddl_trip.SelectedValue == "ALL")
                    {
                        ...
                        DataTable uniqueTrips = rslt.DefaultView.ToTable(true, "RouteName", "TripMeridian");
                        foreach (DataRow dr in uniqueTrips.Rows)
                        {
                            DataRow[] AMPMROWS = rslt.Select("TripMeridian='" + dr["TripMeridian"].ToString() + "'");
                            if (AMPMROWS.Length > 0)
                            {
                                ...
                                newrow["Trip Name"] = AMPMROWS[0]["TripMeridian"].ToString();
                        ...
                        foreach (var column in finaltable.Columns.Cast<DataColumn>().ToArray())
                        {
                            if (finaltable.AsEnumerable().All(dr => dr.IsNull(column)))
                                finaltable.Columns.Remove(column);
                        }
                ...
                else
                {
                    if (ddl_trip.SelectedValue == "ALL")
                    {
                        ...
                        foreach (DataRow dr in rslt.Rows)
                        {
                            ...
                            newrow["SNo"] = i++;
                            newrow["TripTime"] = dr["TripTime"].ToString();
                            newrow["TripMeridian"] = dr["TripMeridian"].ToString();
                            newrow["RouteName"] = dr["RouteName"].ToString();
                            newrow["ActualKMs"] = dr["ActualKMs"].ToString();
                            newrow["GPSKMs"] = dr["GPSKMs"].ToString();
                            newrow["BillingKMs"] = dr["BillingKMs"].ToString();
                            newrow["ChargePerKM"] = dr["ChargePerKM"].ToString();
                            newrow["TotalCharge"] = dr["TotalCharge"].ToString();
                            newrow["Remarks"] = dr["remarks"].ToString();
                            Report.Rows.Add(newrow);
                        }
                        DataRow newTotal = Report.NewRow();
                        newTotal["TripMeridian"] = "Total";
                        double val = 0.0;
                    ...
                    else
                    {
                        ...
                        Report.Columns.Add("ChargePerKM");
                        Report.Columns.Add("TotalCharge").DataType = typeof(double);
                        cmd = new MySqlCommand("SELECT Date_Format(distancestatistics.G_Date,'%m/%d/%Y') AS TripTime,distancestatistics.TripMeridian,distancestatistics.remarks,routetable.RouteName,  distancestatistics.Actual_kms AS ActualKMs, distancestatistics.GPS_kms AS GPSKMs, distancestatistics.Billing_kms AS BillingKMs, distancestatistics.ChargePerKM, distancestatistics.TotalCharge, distancestatistics.Remarks FROM distancestatistics INNER JOIN routetable ON distancestatistics.RouteName_sno = routetable.SNo WHERE (distancestatistics.RouteName_sno = @RouteName_sno) AND (distancestatistics.TripMeridian = @TripMeridian) AND (distancestatistics.UserID = @UserID) AND (distancestatistics.G_Date >= @d1) AND (distancestatistics.G_Date <= @d2) order by distancestatistics.G_Date");
                        cmd.Parameters.Add("@UserID", UserName);
                        cmd.Parameters.Add("@PlantName", ddl_plant.SelectedValue);
                        cmd.Parameters.Add("@RouteName_sno", ddl_routes.SelectedValue);
                        cmd.Parameters.Add("@TripMeridian", ddl_trip.SelectedValue);
